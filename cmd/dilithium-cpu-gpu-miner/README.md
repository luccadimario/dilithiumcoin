...
Download `dilithium-cpu-gpu-miner` from the [releases page](https://github.com/luccadimario/dilithiumcoin/releases), then:

```bash
chmod +x dilithium-cpu-gpu-miner
./dilithium-cpu-gpu-miner --address YOUR_DLT_ADDRESS
```

The miner starts an embedded node automatically. Use `--peer` to connect to the network:

```bash
./dilithium-cpu-gpu-miner --address YOUR_ADDRESS --peer seed.dilithium.network:5000
```
...
Run `./dilithium-cpu-gpu-miner --benchmark` to measure your hardware.
...
```bash
go build ./cmd/dilithium-cpu-gpu-miner
```
